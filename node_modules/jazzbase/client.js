(function(){
    ...
    function processTask(task, returns) {
        if (task.io == 'get') {
            return ajax({channel:task.channel, key:task.key, returns:returns});

        } else if (task.io == 'set') {
            return ajax({channel:task.channel, key:task.key, data:task.data, returns:returns});

        } else if (task.io == 'push') {
            return ajax({channel:task.channel, key:task.key, push:task.data, returns:returns});
        }
    }
